...
class OrbitDataset(Dataset):
    ...
    def __getitem__(self, idx):
        with h5py.File(self.h5_path, 'r') as f:
            ...
            # Extract underlying target parameters
...
def train_kepler_operator(dataset_path="orbits_data.h5", epochs=150, batch_size=64, model_save_path = "kepler_operator_best.pth"):
    ...
    # --- SMART CHECKPOINT LOADER & WARMUP BYPASS ---
    WARMUP_EPOCHS = 30  # Default to 30 epochs of parameter anchoring
    if os.path.exists(model_save_path):
        print(f"Loading existing model weights from {model_save_path}...")
        try:
            model.load_state_dict(torch.load(model_save_path, map_location=device, weights_only=True))
            # Proactively skip parameter warmup since loaded weights already have physical scales locked in
            warmup_epochs = 0
            print("→ Success! Skipping warmup phase to preserve pre-trained phase alignments.")
        except Exception as e:
            print(f"→ Warning: Failed to load state dict due to structural differences: {e}")
            print("→ Starting training fresh with standard warmup.")
    ...
    for epoch in range(epochs):
        ...
        for noisy, clean, targets in train_loader:
            ...
            # Predict the normalized coordinates using differentiable Kepler mechanics
            pred_positions = model(noisy)
            true_positions = clean[:, :, 0:2]
            
            # --- Extract predictions from the model to calculate supervised parameter losses ---
            raw_guess = model.transformer(noisy)
            pred_a_norm = 0.5 + torch.nn.functional.softplus(raw_guess[:, 0:1]) * 3.0
            pred_e_norm = torch.sigmoid(raw_guess[:, 1:2]) * 0.91
            pred_om_norm = raw_guess[:, 2:3] * torch.pi
            pred_m0_norm = raw_guess[:, 3:4] * torch.pi
            pred_M_norm = 1e-6 + torch.nn.functional.softplus(raw_guess[:, 4:5]) * 2.0
            ...
            # --- SUPERVISED PARAMETER LOSS (With wrapping protection for angles) ---
            loss_param = (
            ...
            # --- COORDINATE & RADIUS LOSS ---
            loss_coord = criterion(pred_positions, true_positions)
            pred_radii = torch.norm(pred_positions, dim=-1)
            true_radii = torch.norm(true_positions, dim=-1)
            loss_radius = criterion(pred_radii, true_radii)
            
            # --- TWO-STAGE SCHEDULER LOGIC ---
            if epoch < WARMUP_EPOCHS:
                # Stage 1: Focus purely on mapping physical elements to prevent radial collapse
                loss = loss_param
            else:
                # Stage 2: Train coordinates & radius while maintaining a small parameters anchor
                loss = loss_coord + 3.0 * loss_radius + 0.1 * loss_param
            
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            
            train_loss += loss_coord.item()
        ...
        with torch.no_grad():
            for noisy, clean, targets in val_loader:
                ...
                # Reverse the model's starvation-proof physical boundary mappings
                pred_a_norm = 0.5 + torch.nn.functional.softplus(raw_guess[:, 0]) * 3.0
                pred_e_norm = torch.sigmoid(raw_guess[:, 1]) * 0.91
                pred_M_norm = 1e-6 + torch.nn.functional.softplus(raw_guess[:, 4]) * 2.0
        ...
        with torch.no_grad():
            ...
            # Pull diagnostic parameters specifically for the title card
            raw_anchor_guess = model.transformer(anchor_noisy_batched)
            p_a = 0.5 + torch.nn.functional.softplus(raw_anchor_guess[0, 0]) * 3.0
            p_e = torch.sigmoid(raw_anchor_guess[0, 1]) * 0.91
        ...
        ax.set_title(f"Epoch {epoch+1:03d} Trajectory Fit {'[Warmup Phase]' if epoch < WARMUP_EPOCHS else '[Trajectory Phase]'}\n"
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "kepler_operator_best.pth")
            saved_status = "[Checkpoint Saved]"
            
        if (epoch + 1) % 5 == 0 or saved_status:
            current_lr = optimizer.param_groups[0]['lr']
            print(f"Epoch {epoch+1:03d}/{epochs} | LR: {current_lr:.1e} | Train MSE: {train_loss:.6f} | Val MSE: {val_loss:.6f} {saved_status}")
